    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            'group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar flex min-h-screen w-full', // v3-safe (min-h-svh -> min-h-screen)
            className
          )}
          {...props}
        >
...
function Sidebar({
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* The gap that pushes content right/left on desktop */}
      <div
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn('size-7', className)} // size-7 -> h-7 w-7
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
const sidebarMenuButtonVariants = cva(
  [
    ...
    // === Tailwind v3 equivalents for the v4-only utilities ===
    // when the sidebar is collapsed to icon, force square button
  ...
  ].join(' '),
  {
    variants: {
      ...
      size: {
        ...
        // in v4 this was `p-0!`; in v3 use the important variant prefix `!`
